...
(deftest sort-filter-and-truncation-test
  (let [units [{:ns 'b.core :var 'z :kind :defn-arity :arity 1 :dispatch nil :cc 4 :loc 8 :cc-risk (sut/cc-risk 4)}
    ...
    (is (= :cc (sut/effective-bar-metric {})))
    (is (= :loc (sut/effective-bar-metric {:sort :loc})))
    (is (= :loc (sut/effective-bar-metric {:sort :ns :bar :loc})))
    (is (= :cc (sut/effective-bar-metric {:sort :loc :bar :cc})))

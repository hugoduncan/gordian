...
(defn- run-validators [command opts]
  (or (validate-output-modes opts)
      (when-let [validate (:validate (command-definition command))]
        (validate opts))))

(defn parse-args [raw-args]
  (let [raw-args      (or raw-args [])
        command-token (when (command-token? (first raw-args)) (first raw-args))
        command       (resolve-command command-token)
        command-args  (if command-token (rest raw-args) raw-args)]
    (if command
      (let [{:keys [args opts]} (cli/parse-args command-args {:spec (:spec (command-definition command))})]
        (cond
          (:help opts)
          {:help true :help-command command}

          :else
          (let [parsed ((:parse (command-definition command)) {:args args :opts opts})]
            (if (:error parsed)
              parsed
              (or (run-validators command parsed)
                  parsed)))))
      (let [{:keys [args opts]} (cli/parse-args command-args {:spec (:spec (command-definition :analyze))})
            parsed ((:parse (command-definition :analyze)) {:args args :opts opts})]
        (cond
          (:help opts)
          {:help true}
...
(defn- render-commands []
  (let [lines (command-summary-lines)
...
(defn command-help [command]
  (let [{:keys [usage description positional spec examples names display-name]} (command-definition command)
